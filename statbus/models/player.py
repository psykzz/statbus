...
class Player(DBModel):
    ...
    @property
    def rounds(self):
        return Round.select().join(Connection, on=(Connection.round_id == Round.id)).where(Connection.ckey == self.ckey).order_by(Round.id.desc()).limit(10)

    @property
    def notes(self):
        return Message.select().join(Player, on=(Player.ckey == Message.targetckey)).where(Message.targetckey == self.ckey).order_by(Message.id.desc()).limit(10)
